...
public static class DrinkFactory
{
    ...
    public static IPriceable CreateMilkTea(string milkTeaName)
    {
        ...
        // For new items that don't have specific decorators, create a generic one
        var menuItem = StaticMenuData.MilkTeaItems.FirstOrDefault(m => m.Name == milkTeaName);
        if (menuItem != null)
        {
            return new GenericMilkTeaDecorator(baseDrink, menuItem.Name, menuItem.BasePrice, menuItem.Description);
        }
    ...
    public static IPriceable AddTopping(IPriceable drink, string toppingName)
    {
        ...
        // For new toppings that don't have specific decorators, create a generic one
        var toppingItem = StaticMenuData.ToppingItems.FirstOrDefault(t => t.Name == toppingName);
        if (toppingItem != null)
        {
            return new GenericToppingDecorator(drink, toppingItem.Name, toppingItem.BasePrice, toppingItem.Description);
        }
